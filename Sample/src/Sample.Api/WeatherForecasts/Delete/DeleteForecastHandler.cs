...
{
    Task<OneOf<Success, NotFoundMessage>> DeleteAsync(Ulid id, CancellationToken ct);
}

public class DeleteForecastHandler(ICudRepository<SampleDbContext, WeatherForecast> repository) : IDeleteForecastHandler
{
    public async Task<OneOf<Success, NotFoundMessage>> DeleteAsync(Ulid id, CancellationToken ct)
    {
