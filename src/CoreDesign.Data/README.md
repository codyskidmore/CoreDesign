...
- `CoreDesignDbContext` - Abstract `DbContext` base class your `DbContext` must inherit from instead of `DbContext` directly. Its `ConfigureConventions` override normalizes every `DateTime`/`DateTime?` property in the model, not just `BaseEntity`'s `CreatedAt`/`UpdatedAt`, to `DateTimeKind.Utc` on both write and read, regardless of the `Kind` of the value assigned. This keeps entities working across providers with different `DateTime` strictness (PostgreSQL's Npgsql provider rejects non-UTC values for `timestamp with time zone` columns; SQL Server's `datetime2` does not validate `Kind` at all).
