...
A `DispatchProxy`-based proxy middleware is also included for cases where the proxy approach is preferred or migration from a previous version is not yet complete.
...
The proxy middleware's `[TruncateLog]` attribute is not supported by the generator. The equivalent control belongs in the structured logging configuration, where limits apply consistently across all sinks and can be adjusted per environment without a code change.
...
For larger applications, implement the `ILoggable` marker interface on any class to opt it into automatic logging registration:
...
|---|---|---|
| Boilerplate | One attribute on the interface | `ILoggable` on the implementation |
| Async handling | Direct `await` | Runtime `MakeGenericMethod` |
| AOT compatibility | Yes | No |
| Performance | Direct method calls | `MethodInfo.Invoke` per call |
...
`[TruncateLog]` is not supported by the generator. Use Serilog's `Destructure` configuration in `appsettings.json` instead. It operates at the sink level, applies uniformly, and can be tuned per environment without a code change or rebuild.

**Proxy Infrastructure Retained**

`LoggingMiddleware<T>`, `ILoggable`, and all related proxy infrastructure remain in the package. Existing projects using the proxy do not need to migrate.
