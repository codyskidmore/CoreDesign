...
4. [CoreDesign.SeedTool](#4-coredesignseedtool)
5. [CoreDesign.Logging](#5-coredesignlogging)
6. [CoreDesign.ExceptionHandling](#6-coredesignexceptionhandling)
7. [CoreDesign.Identity](#7-coredesignidentity)
   - [CoreDesign.Identity.Server](#71-coredesignidentityserver)
   - [CoreDesign.Identity.Client](#72-coredesignidentityclient)
8. [Azure Entra (Production Authentication)](#8-azure-entra-production-authentication)
9. [Shared appsettings Configuration](#9-shared-appsettings-configuration)
10. [Local NuGet Development](#10-local-nuget-development)
11. [Sample Application](#11-sample-application)
    - [Project Overview](#111-project-overview)
    - [Prerequisites and Setup](#112-prerequisites-and-setup)
    - [Sample.Identity.Web](#113-sampleidentityweb)
    - [Sample.Api](#114-sampleapi)
    - [Sample.Blazor](#115-sampleblazor)
    - [Sample.Data.MigrationService](#116-sampledatamigrationservice)
    - [Deployment](#117-deployment)
12. [Vertical Slice Architecture Reference](#12-vertical-slice-architecture-reference)
...
| `CoreDesign.SeedTool` | `src/CoreDesign.SeedTool/` | `dotnet seed` CLI tool for exporting live entity data to seed files and comparing seed files against a live database |
| `CoreDesign.Logging` | `src/CoreDesign.Logging/` | Compile-time logging decorators generated by Roslyn. Place `[LoggingDecorator]` on any interface for automatic, structured logging with no reflection overhead |
| `CoreDesign.ExceptionHandling` | `src/CoreDesign.ExceptionHandling/` | Global unhandled exception handling for ASP.NET Core. Place `[ProblemMapping]` on any exception type for a compile-time dispatch table to RFC 7807 `ProblemDetails` responses. See [src/CoreDesign.ExceptionHandling/README.md](src/CoreDesign.ExceptionHandling/README.md) for full documentation |
...
- `AuditInterceptor`: EF Core `SaveChangesInterceptor` that sets audit fields on every save. On insert it sets `CreatedAt`, `CreatedBy`, `UpdatedAt`, and `UpdatedBy`. On update it sets `UpdatedAt` and `UpdatedBy` only. `CreatedBy` is never modified on update.
...
**3. Implement `ICurrentUserAccessor`**
...
For migration services and background workers, use the built-in `SystemUserAccessor` (returns `Guid.Empty`).

**4. Register services**

Call `AddCoreDesignData<T>` before registering the DbContext. Wire the `AuditInterceptor` via `AddInterceptors` when configuring the DbContext pool:
...
**5. Inherit from `CoreDesignDbContext`**
...
Audit fields are set automatically by the interceptor. No `userId` parameter is needed:
...
// Soft delete parent and all children in the EF navigation graph
await cudRepository.SoftDeleteCascadeAsync(id, cancellationToken);

// Hard delete (physically removes the row; works even if soft-deleted)
await cudRepository.HardDeleteAsync(id, cancellationToken);

// Hard delete parent and all children in the EF navigation graph
...
- Cascade methods walk the EF Core navigation graph in memory and load child collections before acting on them. Large graphs increase memory usage proportionally.
...
- Seeded entities with explicit audit values in JSON are preserved as-is. The interceptor only fills in audit fields when `CreatedAt` is at its default value.
...
## 4. CoreDesign.SeedTool

`CoreDesign.SeedTool` is a `dotnet` global or local tool (`dotnet seed`) for exporting live entity data to `MigrationWorker`-compatible seed files and comparing seed files against a live database. It works with any project that uses `CoreDesign.Data`.
...
Run from your repository root. This creates `coredesign.seedtool.json`:

```
dotnet seed setup
```

Setup prompts for the assembly path, seed data directory, entity list, and connection mode. After setup, no extra flags are needed:
...
The tool supports four connection modes, chosen during setup.

#### Mode 1: Template + user secrets (Aspire apps)

Stores a connection string template in `coredesign.seedtool.json`. The password is read at runtime from a project's user secrets. No credential is ever written to the config file.

This is the recommended mode for Aspire apps where the connection string is managed by the AppHost. Store the connection string template in AppHost user secrets alongside the password parameter:
...
The tool reads the template from user secrets, resolves any `{ParameterName}` placeholders by looking up `Parameters:ParameterName` in the same config stack (matching Aspire's own substitution behavior), and connects without any additional flags.

#### Mode 2: App config (non-Aspire apps)

Reads the connection string from the app's own `appsettings.json`, `appsettings.Development.json`, and user secrets, exactly as the app does at runtime. Nothing in `coredesign.seedtool.json` can go stale.
...
$env:SEEDTOOL_PASSWORD = "mysecret"
```

#### Password resolution order

When `{password}` appears in the connection string, the tool resolves it in this order:

1. `--password` flag
2. `SEEDTOOL_PASSWORD` environment variable
3. `userSecrets` config (project + passwordKey)

`--connection` bypasses all resolution.

### AppHost wiring (Aspire)

For the tool to read the connection string from AppHost user secrets and have Aspire inject the same string into the running app, wire the AppHost using `AddConnectionString` and `WithConnectionStringRedirection`:
...
This pattern lets external tools (DataGrip, seed tool, database migrations) all use the same static connection string from user secrets while Aspire injects it into the running app at runtime.

### Seed factory

Add one class to your application project so the tool can create your `DbContext` from a connection string:
...
The factory is discovered by reflection at runtime. It must have a public parameterless constructor.
...
Seed files are named by the entity's fully qualified type name. Renaming or moving an entity class silently breaks the match. Run `dotnet seed diff` after any rename to surface the mismatch, then `dotnet seed export` to regenerate under the correct names.
...
## 5. CoreDesign.Logging

`CoreDesign.Logging` provides compile-time logging decorators generated by a Roslyn source generator. Place `[LoggingDecorator]` on any interface and the generator produces a complete decorator class at compile time. Implementations stay free of log statements while still producing structured, consistent log output for every operation.
...
Generic interfaces are fully supported. The decorator carries the same type parameters and constraint clauses as the interface:
...
Register each interface and implementation normally, then call the generated extension method once:
...
`DecorateWithLogging()` is generated alongside the decorators and wraps every marked interface in one call. Adding a new interface marked with `[LoggingDecorator]` is picked up automatically on the next build with no changes to the startup call.
...
Apply `[Suppress]` to a method to skip all logging for it entirely. The generated decorator passes the call straight through to the inner implementation with no log entries written.
...
The generated decorator delegates `this[key]` and `Count` directly to the inner implementation with no log entries. `ExistsAsync` is logged normally.

#### Controlling Log Output Size

Log output size control belongs in the structured logging configuration, where limits apply consistently across all sinks and can be adjusted per environment without a code change.

With Serilog, add a `Destructure` block to `appsettings.json`:
...
`ToMaximumDepth` caps how many levels deep Serilog traverses when destructuring an object. Without this setting, a deeply nested object graph such as an EF Core entity with navigation properties will cause Serilog to stream an enormous payload into every sink and can hang the application. `ToMaximumStringLength` truncates any string captured during destructuring. `ToMaximumCollectionCount` caps how many elements are captured from arrays and collections. All three settings are tunable per environment using `appsettings.Development.json` or `appsettings.Production.json` without touching source code.
...
### CoreDesign.Logging vs Serilog and Serilog.Enrichers.Sensitive

These tools operate at fundamentally different layers and are not alternatives to each other.

**Serilog** is a logging pipeline. It handles formatting, enrichment, filtering, and routing entries to sinks (files, Seq, Application Insights, etc.). It has no concept of application code and no mechanism to intercept method calls.

**Serilog.Enrichers.Sensitive** works inside that pipeline. It scans already-formed log messages for patterns and masks matches before they reach a sink. It does not know what triggered the log entry or where in the application it came from.

**CoreDesign.Logging** operates one layer up, at the point where application code calls any decorated service. It intercepts every method invocation before it happens, which gives it three things the others cannot offer:
...
- **Zero instrumentation in application classes.** Classes contain no log statements and no awareness of observability at all.

The right mental model is that these are complementary:

| Layer | Tool | Responsibility |
|---|---|---|
| Application class boundary | CoreDesign.Logging | Intercepts calls on any decorated interface, logs method invocations and results |
...
Using all three together gives intentional logging at the application layer, flexible output routing, and a defensive backstop for third-party libraries and ad-hoc log statements outside the decorator.
...
## 6. CoreDesign.ExceptionHandling
...
Ordinary error conditions, not found, validation, and the like, should be represented as a returned value, not a thrown exception. The `GetById` slice under `WeatherForecasts/` does this with `OneOf<WeatherForecast, NotFoundMessage>`, handled explicitly with `.Match()` in the endpoint. `CoreDesign.ExceptionHandling` is not a substitute for that pattern; throwing a custom exception to represent an expected condition just to avoid a `.Match()` call adds needless overhead and models a normal outcome as an error.

What `CoreDesign.ExceptionHandling` is for is the exception nobody planned for. The `SimulateUnhandledException` slice demonstrates exactly that: it throws a plain, unmapped `InvalidOperationException`, no `[ProblemMapping]` in sight, to exercise the package's zero-config fallback path:
...
With no mapping for `InvalidOperationException` anywhere in the compilation, the global handler falls back to `NullProblemDetailsMapper`: a generic RFC 7807 500 response, with `detail` populated only in `Development`. No endpoint author had to write a `try`/`catch`, and no domain type had to be invented to carry the error.

Startup wiring lives in `Configuration.cs` (`AddCoreDesignExceptionHandling()` / `AddGeneratedProblemMappings()`) and `App.cs` (`app.UseExceptionHandler()`, registered first in the pipeline so it wraps everything downstream).
...
### 7.1 CoreDesign.Identity.Server
...
### 7.2 CoreDesign.Identity.Client
...
## 8. Azure Entra (Production Authentication)
...
## 9. Shared appsettings Configuration
...
The PostgreSQL container password is supplied via .NET user secrets so it is never stored in source control. Run from the repository root:
...
The main ASP.NET Core REST API. It implements Vertical Slice Architecture (VSA): each HTTP operation is fully self-contained in its own folder. See [Section 12](#12-vertical-slice-architecture-reference) for the full architecture analysis.
...
| `Configuration.cs` | Registers services: database, identity, authorization, CORS, output caching, and telemetry. Also registers `BearerSecurityTransformer`, which annotates the OpenAPI document with the Bearer security scheme and automatically excludes anonymous endpoints. |
...
| `SampleCurrentUserAccessor.cs` | Implements `ICurrentUserAccessor` by reading the `oid` claim from `IHttpContextAccessor`. Returns `Guid.Empty` when no HTTP context is present. |
...
| `DELETE /WeatherForecasts/{id}` | `weather:write` | Soft-deletes via `ICudRepository.SoftDeleteAsync`, evicts cache |
...
Classes in Sample.Api contain no log statements. All invocation logging is handled by `CoreDesign.Logging` source-generated decorators.
...
All generated decorators are registered in one call in `ModuleConfig.cs`, after the individual handler registrations:
...
`DecorateWithLogging()` is generated alongside the decorator classes. Adding a new handler interface marked with `[LoggingDecorator]` is sufficient to get consistent, structured log output automatically on the next build. No changes to `Configuration.cs` or `ModuleConfig.cs` are required.

Log output size is controlled through Serilog's `Destructure` configuration in `appsettings.json` rather than per-method attributes:
...
These settings apply uniformly across all handlers and all sinks, and the limits can be overridden per environment in `appsettings.Development.json` or `appsettings.Production.json` without changing source code.
...
### 11.6 Sample.Data.MigrationService
...
`Program.cs` registers `MigrationWorker<SampleDbContext>` and its OpenTelemetry `ActivitySource`. `SystemUserAccessor` is registered because the migration service has no authenticated user:
...
With managed identity, Npgsql has no `Authentication=` connection-string keyword equivalent to SQL Server's; acquire an Entra ID access token in code (via `Azure.Identity`) and pass it as the `Password`, or use `NpgsqlDataSourceBuilder.UsePeriodicPasswordProvider` so the migration service always has a fresh token.

---

## 12. Vertical Slice Architecture Reference
...
New package. A single global `IExceptionHandler` for ASP.NET Core that maps unhandled exceptions to RFC 7807 `ProblemDetails` responses via a compile-time `[ProblemMapping]` dispatch table generated by a Roslyn source generator. See Section 6 for full documentation.
...
`LoggingMiddleware<T>`, `ILoggable`, `[TruncateLog]`, and the `AddWithLogging` extension methods have been removed. These were deprecated in 1.1.0 when the source generator was introduced and have been absent from all new projects since then.
...
To control log output size, use Serilog's `Destructure` configuration in `appsettings.json` instead of `[TruncateLog]`:
...
`[Redact]` and `[Suppress]` are unchanged and continue to work exactly as before.

**Breaking Changes**

- `LoggingMiddleware<T>` class removed
- `ILoggable` interface removed
- `TruncateLogAttribute` removed
- `AddWithLogging<TInterface, TImplementation>()` extension removed
- `AddWithLogging(Assembly, ServiceLifetime)` extension removed

---

### CoreDesign.Data 1.0.3

Complete audit infrastructure added. `AuditInterceptor` sets `CreatedAt`, `CreatedBy`, `UpdatedAt`, and `UpdatedBy` automatically on every `SaveChanges`. Explicit audit values in seeded JSON are preserved. `ICurrentUserAccessor`, `SystemUserAccessor`, `CoreDesignDataExtensions.AddCoreDesignData<T>()`, and `CoreDesignDbContext` are also new in this release. See Section 3 for full documentation.

### CoreDesign.Shared 1.1.0

Dependency update: `Aspire.Hosting.AppHost` updated from 13.3.2 to 13.4.2.

### CoreDesign.Logging 1.1.1

Dependency update: `CoreDesign.Shared` updated from 1.0.2 to 1.1.0.

### CoreDesign.Identity.Server 1.1.0

Dependency update: `System.IdentityModel.Tokens.Jwt` updated from 8.18.0 to 8.19.1.

### CoreDesign.Identity.Client 1.1.0

Minor release for version alignment.

---

### Sample Application (Current)

**Sample.Api**: Reorganized to full Vertical Slice Architecture. Each HTTP operation is self-contained in its own folder with operation-scoped request/response types and direct repository injection. Each handler interface is marked with `[LoggingDecorator]` and all generated decorators are wired up in `ModuleConfig.cs` with a single `services.DecorateWithLogging()` call.
...
A companion `LoggingDecoratorExtensions.g.cs` is also generated, containing a `DecorateWithLogging()` extension method on `IServiceCollection`. A single call during startup registers all decorators:
...
The generator fully supports generic interfaces. The generated decorator class carries the same type parameters and constraint clauses as the interface, and `DecorateWithLogging()` handles open-generic registration automatically.

**AOT Compatible**

Generated decorators contain no reflection, no `DispatchProxy`, and no `MakeGenericMethod` calls. They are fully compatible with .NET Native AOT.

**Log Output Size Control**

Use Serilog's `Destructure` configuration in `appsettings.json` to control log output size. It operates at the sink level, applies uniformly, and can be tuned per environment without a code change or rebuild.

**Sample Application Updated**

Each handler interface in Sample.Api is now marked with `[LoggingDecorator]`. The `ILoggable` marker interface has been removed from implementations. The `AddWithLogging(assembly)` call in `Configuration.cs` was replaced with `services.DecorateWithLogging()` in `ModuleConfig.cs`.
