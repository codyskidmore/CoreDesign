...
The `GetByIdOrThrow` slice under `WeatherForecasts/` demonstrates the pattern next to the existing `GetById` slice, which returns `OneOf<WeatherForecast, NotFoundMessage>` and handles it explicitly with `.Match()`. `GetByIdOrThrow` throws a `[ProblemMapping(404)] WeatherForecastNotFoundException` from the handler instead and relies entirely on the global handler for the response, no `.Match()` call in the endpoint:
